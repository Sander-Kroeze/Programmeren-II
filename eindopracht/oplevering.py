...
class Board:
    ...
    def Extract(self, number, lst): 
        return [item[number] for item in lst] 

    def is_row_full(self, row):
        """"
            Kijkt of een rij vol zit.
        """        
        count = 0
        isListFull = self.Extract(row, self.data)
        for j in isListFull:
            if j == ' ':
                return
            else:
        ...
        while True :
            if px != "human" :
                if random.random() < 0.20:
                    print('de AI doet een random zet!')
                    randomNumber = randrange(7)
                    if self.is_row_full(randomNumber) == True: 
                        print('De gekozen rij (rij', randomNumber, ') is al vol, je moet een beurt overslaan!')
                    self.add_move(randomNumber, "X")
                else:
                    self.add_move(px.next_move(self), "X")
            else :
                users_col = -1
                while self.allows_move( users_col ) == False:
                    users_col = input("X: Kies een kolom: ")
                self.add_move(users_col, "X")
            ...
            if po != "human":
                if random.random() < 0.20:
                    ...
                    randomNumber = randrange(7)
                    if self.is_row_full(randomNumber) == True:
                        print('De gekozen rij (rij', randomNumber,'), je moet een beurt overslaan!')
                    self.add_move(randomNumber, "O")
                else:
                    self.add_move(px.next_move(self), "O")
            else :
                users_col = -1
                while self.allows_move( users_col ) == False:
